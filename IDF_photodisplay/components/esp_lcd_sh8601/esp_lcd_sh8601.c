...
static esp_err_t panel_sh8601_draw_bitmap(esp_lcd_panel_t* panel, int x_start, int y_start,
...
esp_err_t esp_lcd_new_panel_sh8601(const esp_lcd_panel_io_handle_t io,
  ...
  *ret_panel = &(sh8601->base);
  ...
  ESP_LOGI(TAG, "LCD panel create success, version: %d.%d.%d", ESP_LCD_SH8601_VER_MAJOR,
           ESP_LCD_SH8601_VER_MINOR, ESP_LCD_SH8601_VER_PATCH);
...
static esp_err_t tx_param(sh8601_panel_t* sh8601, esp_lcd_panel_io_handle_t io, int lcd_cmd,
...
static esp_err_t tx_color(sh8601_panel_t* sh8601, esp_lcd_panel_io_handle_t io, int lcd_cmd,
                          const void* param, size_t param_size) {
  if (sh8601->flags.use_qspi_interface) {
    lcd_cmd &= 0xff;
    lcd_cmd <<= 8;
...
static esp_err_t panel_sh8601_init(esp_lcd_panel_t* panel) {
  ...
  for (int i = 0; i < init_cmds_size; i++) {
    ...
    if (is_cmd_overwritten) {
      ESP_LOGW(TAG,
               "The %02Xh command has been used and will be overwritten by external initialization "
               "sequence",
               init_cmds[i].cmd);
    }

    ESP_RETURN_ON_ERROR(
        tx_param(sh8601, io, init_cmds[i].cmd, init_cmds[i].data, init_cmds[i].data_bytes), TAG,
        "send command failed");
